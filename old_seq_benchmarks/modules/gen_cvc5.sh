...
do
    ...
    then
        ...
        if [ $ret -eq 0 ]; then
            ...
            start=$(date +%s)
            timeout 20 $boogie $bm_bpl -monomorphize /env:2 /proverLog:$bm_smt2 /proverOpt:PROVER_PATH=$cvc5 /proverOpt:SOLVER=CVC4 /trace -doModSetAnalysis > $bm_dir_single/boogie_cvc5.log 2>&1
            end=$(date +%s)
            take=$(( end - start ))
