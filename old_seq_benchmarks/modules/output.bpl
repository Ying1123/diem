...
procedure {:timeLimit 40} $1_ChainId_initialize$verify(_$t0: int, _$t1: int) returns ()
{
    // declare local variables
    var $t2: bool;
    var $t3: int;
    ...
    var $1_ChainId_ChainId_$memory#107: $Memory $1_ChainId_ChainId;
    $t0 := _$t0;
    $t1 := _$t1;
    ...
    // @106 := save_mem(DiemTimestamp::CurrentTimeMicroseconds) at /home/ying/diem/language/diem-framework/modules/ChainId.move:28:9+34
    $1_DiemTimestamp_CurrentTimeMicroseconds_$memory#106 := $1_DiemTimestamp_CurrentTimeMicroseconds_$memory;

    // @107 := save_mem(ChainId::ChainId) at /home/ying/diem/language/diem-framework/modules/ChainId.move:28:9+34
    $1_ChainId_ChainId_$memory#107 := $1_ChainId_ChainId_$memory;
...
L0:
    ...
    assert {:msg "assert_failed(7,942,949): caller does not have permission to modify `ChainId::ChainId` at given address"}
      $1_ChainId_ChainId_$modifies[$t0];
    ...
    if ($ResourceExists($1_ChainId_ChainId_$memory, $t0)) {
        call $ExecFailureAbort();
    } else {
        $1_ChainId_ChainId_$memory := $ResourceUpdate($1_ChainId_ChainId_$memory, $t0, $t13);
    }
    if ($abort_flag) {
...
L2:
    ...
    // assert Not(Not(DiemTimestamp::$is_genesis[@106]())) at /home/ying/diem/language/diem-framework/modules/DiemTimestamp.move:140:9+51
    assume {:print "$at(17,5622,5673)"} true;
    assert {:msg "assert_failed(17,5622,5673): function does not abort under this condition"}
      !!$1_DiemTimestamp_$is_genesis($1_DiemTimestamp_CurrentTimeMicroseconds_$memory#106);
    ...
    // assert Not(exists[@107]<ChainId::ChainId>($t4)) at /home/ying/diem/language/diem-framework/modules/ChainId.move:31:9+66
    assume {:print "$at(7,1259,1325)"} true;
    assert {:msg "assert_failed(7,1259,1325): function does not abort under this condition"}
      !$ResourceExists($1_ChainId_ChainId_$memory#107, $t4);
...
L3:
    ...
    // assert Or(Or(Not(DiemTimestamp::$is_genesis[@106]()), Neq<address>(Signer::spec_address_of[]($t0), a550c18)), exists[@107]<ChainId::ChainId>($t4)) at /home/ying/diem/language/diem-framework/modules/ChainId.move:25:5+384
    assume {:print "$at(7,989,1373)"} true;
    assert {:msg "assert_failed(7,989,1373): abort not covered by any of the `aborts_if` clauses"}
      ((!$1_DiemTimestamp_$is_genesis($1_DiemTimestamp_CurrentTimeMicroseconds_$memory#106) || !$IsEqual'address'($1_Signer_spec_address_of($t0), 173345816)) || $ResourceExists($1_ChainId_ChainId_$memory#107, $t4));

    // assert Or(Or(And(Not(DiemTimestamp::$is_genesis[@106]()), Eq(1, $t6)), And(Neq<address>(Signer::spec_address_of[]($t0), a550c18), Eq(2, $t6))), And(exists[@107]<ChainId::ChainId>($t4), Eq(6, $t6))) at /home/ying/diem/language/diem-framework/modules/ChainId.move:25:5+384
    assert {:msg "assert_failed(7,989,1373): abort code not covered by any of the `aborts_if` or `aborts_with` clauses"}
      (((!$1_DiemTimestamp_$is_genesis($1_DiemTimestamp_CurrentTimeMicroseconds_$memory#106) && $IsEqual'num'(1, $t6)) || (!$IsEqual'address'($1_Signer_spec_address_of($t0), 173345816) && $IsEqual'num'(2, $t6))) || ($ResourceExists($1_ChainId_ChainId_$memory#107, $t4) && $IsEqual'num'(6, $t6)));
