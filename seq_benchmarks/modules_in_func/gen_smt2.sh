    ...
    then
        ...
        if [ $ret -eq 0 ]; then
			...
			do
				...
				elif [ "$option" = "cvc5" ] || [ "$option" = "seqArray_array" ]; then
				...
				elif [ "$option" = "andy_default" ]; then
					# cvc5_seqArray --pre-skolem-quant --no-dt-share-sel
					timeout 60 $boogie $bm_bpl -monomorphize /env:2 -proverLog:$option_dir/@PROC@.smt2 /proverOpt:PROVER_PATH=$cvc5_seqArray /proverOpt:O:pre-skolem-quant=true /proverOpt:O:no-dt-share-sel=true /proverOpt:SOLVER=CVC5 /trace -doModSetAnalysis > $option_dir/boogie.log 2>&1
				elif [ "$option" = "andy_strings" ]; then
					# cvc5_seqArray --pre-skolem-quant --no-dt-share-sel
					# --strings-exp
					timeout 60 $boogie $bm_bpl -monomorphize /env:2 -proverLog:$option_dir/@PROC@.smt2 /proverOpt:PROVER_PATH=$cvc5_seqArray /proverOpt:O:strings-exp=true /proverOpt:O:pre-skolem-quant=true /proverOpt:O:no-dt-share-sel=true /proverOpt:SOLVER=CVC5 /trace -doModSetAnalysis > $option_dir/boogie.log 2>&1
				elif [ "$option" = "seqArray" ]; then
					# cvc5_seqArray --pre-skolem-quant --no-dt-share-sel
					# --strings-exp --strings-seq-update=eager
					timeout 60 $boogie $bm_bpl -monomorphize /env:2 -proverLog:$option_dir/@PROC@.smt2 /proverOpt:PROVER_PATH=$cvc5_seqArray /proverOpt:O:strings-exp=true /proverOpt:O:strings-seq-update=eager /proverOpt:O:pre-skolem-quant=true /proverOpt:O:no-dt-share-sel=true /proverOpt:SOLVER=CVC5 /trace -doModSetAnalysis > $option_dir/boogie.log 2>&1
				else
