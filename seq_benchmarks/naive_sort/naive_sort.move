...
module 0x42::TestNaiveSort {
	...
	public fun verify_sort(v: &mut vector<u64>) {
        ...
        while ({
			spec {
		        ...
		        assert exists p : vector<u64> : forall k in 0..len(v): v[k] == old(v)[p[k]] && (forall num in 0..len(v): exists ind in 0..len(v): p[ind] == num);
			};
			(j < vlen)
		}) {
	...
	spec verify_sort {
		...
		ensures exists p : vector<u64> : forall k in 0..len(v): v[k] == old(v)[p[k]] && (forall num in 0..len(v): exists ind in 0..len(v): p[ind] == num);
	}
}
