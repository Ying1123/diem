...
L11:
    ...
    assume (!$IsEqual'address'($1_Signer_spec_address_of($t0), 0) && $IsEqual'u8'(2, $t12));
...
L13:
    ...
    assume (!$ResourceExists($1_DiemConfig_DiemConfig'$1_DiemSystem_DiemSystem'_$memory, 173345816) && $IsEqual'u8'(5, $t12));
...
L15:
    ...
    assume ((!$1_DiemTimestamp_$is_operating($1_DiemTimestamp_CurrentTimeMicroseconds_$memory) && $IsEqual'u8'(1, $t12)) || (!$IsEqual'address'($1_Signer_spec_address_of($t0), 0) && $IsEqual'u8'(2, $t12)));
...
L7:
    ...
    assert {:msg "assert_failed(12,3280,3820): abort code not covered by any of the `aborts_if` or `aborts_with` clauses"}
      ((((!$1_DiemTimestamp_$is_operating($1_DiemTimestamp_CurrentTimeMicroseconds_$memory#175) && $IsEqual'u8'(1, $t12)) || (!$IsEqual'address'($1_Signer_spec_address_of($t0), 0) && $IsEqual'u8'(2, $t12))) || ((!$IsEqual'address'($t4, 0) && !$1_DiemSystem_spec_is_validator($1_DiemConfig_DiemConfig'$1_DiemSystem_DiemSystem'_$memory#176, $t4)) && $IsEqual'u8'(2, $t12))) || ((($1_DiemBlock_$get_current_block_height($1_DiemBlock_BlockMetadata_$memory#177) + 1) > 18446744073709551615) && $IsEqual'num'(-1, $t12)));
...
L3:
    ...
    assert {:msg "assert_failed(12,1666,1957): abort code not covered by any of the `aborts_if` or `aborts_with` clauses"}
      (((!$1_DiemTimestamp_$is_genesis($1_DiemTimestamp_CurrentTimeMicroseconds_$memory#129) && $IsEqual'u8'(1, $t4)) || (!$IsEqual'address'($1_Signer_spec_address_of($t0), 173345816) && $IsEqual'u8'(2, $t4))) || ($1_DiemBlock_$is_initialized($1_DiemBlock_BlockMetadata_$memory#130) && $IsEqual'u8'(6, $t4)));
