...
do
    ...
    then
        ...
        if [ $ret -eq 0 ]; then
            ...
            timeout 10 cargo run --release --quiet --package move-prover -- -d $move_dir -d ~/diem/language/move-stdlib/modules $move_dir/$filename.move --use-cvc4 --vector-theory SmtSeq -v debug -k > $bm_dir_single/mvp_seqArray.log 2>&1
            rm *.smt
            rm *.bpl.log
            ...
            start=$(date +%s%N)
            timeout 40 $boogie $bm_bpl -monomorphize /env:2 /proverLog:$bm_smt2 /proverOpt:PROVER_PATH=$cvc5 /proverOpt:O:strings-exp=true /proverOpt:O:strings-seq-update=eager /proverOpt:O:pre-skolem-quant=true /proverOpt:O:no-dt-share-sel=true /proverOpt:SOLVER=CVC5 /trace -doModSetAnalysis > $bm_dir_single/boogie_seqArray.log 2>&1
            end=$(date +%s%N)
            ...
            if [ -f "$bm_smt2" ]; then
                ...
                timeout 40 $cvc5 $bm_dir_single/$bm_smt2 --incremental --strings-exp --strings-seq-update > $bm_dir_single/seqArray.output 2>&1
            fi
        fi
    fi
done
