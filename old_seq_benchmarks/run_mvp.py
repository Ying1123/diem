...
#        wrapper = ['libtool', '--mode=execute', 'valgrind']
    ...
    bin_args = ['/Users/clarkbarrett/fb/diem-barrettcw/target/release/move-prover']
    bin_args += ['-v','debug','-g','-d','/Users/clarkbarrett/fb/diem-barrettcw/language/move-stdlib/modules','-d',benchmark_dir]

    results = open(output_dir+'/results.csv', 'w')
    results.write('benchmark, Solver Time, Exit Status\n')
    for f in files:
        if not f.endswith('move'):
            continue
        command_line = [benchmark_dir+'/'+f,'-o',output_dir+'/'+f+'.bpl']
