...
L7:
    ...
    assume (!$IsEqual'address'($1_Signer_spec_address_of($t0), 173345816) && $IsEqual'u8'(2, $t6));
...
L3:
    ...
    assert {:msg "assert_failed(7,989,1373): abort code not covered by any of the `aborts_if` or `aborts_with` clauses"}
      (((!$1_DiemTimestamp_$is_genesis($1_DiemTimestamp_CurrentTimeMicroseconds_$memory#106) && $IsEqual'u8'(1, $t6)) || (!$IsEqual'address'($1_Signer_spec_address_of($t0), 173345816) && $IsEqual'u8'(2, $t6))) || ($ResourceExists($1_ChainId_ChainId_$memory#107, $t4) && $IsEqual'u8'(6, $t6)));
