...
do
    ...
    then
        ...
        if [ $ret -eq 0 ]; then
            ...
            cargo run --release --quiet --package move-prover -- -d ~/diem/language/move-stdlib/modules $move_dir/$filename.move --use-cvc4 --vector-theory SmtSeq --generate-smt -k > $bm_dir_single/mvp_log 2>&1
            ...
            ~/boogie/Source/BoogieDriver/bin/Debug/netcoreapp3.1/BoogieDriver $bm_bpl -monomorphize /env:2 /proverLog:$bm_smt2 /proverOpt:PROVER_PATH=/home/ying/bin/cvc4 /proverOpt:SOLVER=CVC4 /trace -doModSetAnalysis > $bm_dir_single/boogie_log 2>&1
			if [ -f "$bm_smt2" ]; then
